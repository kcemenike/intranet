...
class DatabaseSeeder extends Seeder
{
    ...
    {                     
        foreach ($this->areas() as $key => $area) {
            ...
            $locals = $area['state']['locals'];
            
            $dbState = (array) DB::table('states')->where([['name', 'LIKE', '%'.explode(' ', $state)[0].'%']])->first(['id']);
            
            foreach ($locals as $key => $local) {
                DB::table('areas')->insert([
                    'state_id' => $dbState['id'] ?? '00',
                    'name' => $local['name'],
                    'short_name' => strtoupper(substr($local['name'], 0, 2)),
                ]); 
            }   
        }
    }
    ...
    { 
        foreach ($this->countries as $shortName => $country) {
